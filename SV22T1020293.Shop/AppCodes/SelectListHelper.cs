...
namespace SV22T1020293.Shop
{
    ...
    public static class SelectListHelper
    {
        ...
        public static async Task<List<SelectListItem>> Categories()
        {
            ...
            var input = new PaginationSearchInput() { Page = 1, PageSize = ApplicationContext.PageSize, SearchValue = "" };
        ...
        public static async Task<List<SelectListItem>> Shippers()
        {
            var list = new List<SelectListItem>()
    {
        new SelectListItem() { Value = "0", Text = "-- Chọn đơn vị giao hàng --" }
    };
            ...
            foreach (var item in result.DataItems)
            {
                list.Add(new SelectListItem()
                {
                    ...
                    Text = $"{item.ShipperName} {(string.IsNullOrWhiteSpace(item.Phone) ? "" : "- " + item.Phone)}"
                });
            }
